...
class PostSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = Post
        fields = ['title', 'content', 'content_type', 'tags', 'date_created', 'date_edited']
    ...
    def validate(self, data):
        ...
        tags = data[tags]
        if len(tags) == 0:
            raise serializers.ValidationError("Post must contain at least one tag")
        if len(tags) > 10:
            raise serializers.ValidationError(f"Exceed maximum amount of tags (10). Given: {len(tags)} tags")    
